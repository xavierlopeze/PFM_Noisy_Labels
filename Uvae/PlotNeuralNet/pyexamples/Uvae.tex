...
\node[canvas is zy plane at x=0] (temp) at (-3,0,0) {\includegraphics[width=8cm,height=8cm]{../examples/fcn8s/apple_pie.jpg}};
...
\draw [connection]  (pool1-east)    -- node {\midarrow} (ccr_b1-west);


\path (pool1-southeast) -- (pool1-northeast) coordinate[pos=2.0] (pool1-top) ;
\path (ccr_b1-south)  -- (ccr_b1-north)  coordinate[pos=2.0] (ccr_b1-top) ;
\draw [copyconnection]  (pool1-northeast)  
-- node {\copymidarrow}(pool1-top)
-- node {\copymidarrow}(ccr_b1-top)
-- node {\copymidarrow} (ccr_b1-north);
...
\draw [connection]  (ccr_b4-east)    -- node {\midarrow} (unpool_b6-west);
